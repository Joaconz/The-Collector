...
const MisSubastasPage = ({ misSubastas }) => {
  const pujasActivas = misSubastas.filter((s) => s.estado === 'ABIERTA');
  const historial = misSubastas.filter((s) => s.estado === 'CERRADA');

  const pujasSuperadas = pujasActivas.filter((s) => s.pujaUsuario < s.pujaLider);
  const pujasLider = pujasActivas.filter((s) => s.pujaUsuario >= s.pujaLider);
      ...
      <div className="max-w-7xl w-full flex flex-col space-y-12">
        ...
        {misSubastas.length > 0 ? (
          <div className="flex flex-col space-y-12">
            ...
            {/* Sección 1: Pujas Superadas (Acción Requerida) */}
            ...
            {/* Sección 2: Puja Líder (posición asegurada) */}
            {pujasLider.length > 0 && (
